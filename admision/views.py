...
def admisiones(request):
    return render(request, 'ventanas/admisiones.html')

def pacientes_espera(request):
    pacientes = paciente.objects.filter(estado='en espera') 
    return render(request, 'triage/triage.html', {'pacientes': pacientes})

# def eliminar_paciente(request, paciente_id):
#     paciente = get_object_or_404(paciente, id=paciente_id)
#     paciente.delete()  # Elimina el registro
#     return redirect('ventanas/paciente_agregar.html')  # Cambia esto por el nombre de la vista a la que quieres redirigir